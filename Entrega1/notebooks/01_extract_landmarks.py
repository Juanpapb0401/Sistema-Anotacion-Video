import cv2; import mediapipe as mp; import pandas as pd; import os
...
for video_file in os.listdir(INPUT_PATH):
    if not video_file.endswith(('.mp4', '.mov')): continue
    video_path = os.path.join(INPUT_PATH, video_file)
    video_name = os.path.splitext(video_file)[0]
    csv_path = os.path.join(OUTPUT_PATH, f"{video_name}.csv")

    if os.path.exists(csv_path):
        print(f"  Saltando '{video_file}', ya procesado."); continue
    ...
    if landmarks_data:
        ...
        print(f"  Landmarks guardados para '{video_file}'.")

pose.close()
print("\n Extracción de landmarks completada.")